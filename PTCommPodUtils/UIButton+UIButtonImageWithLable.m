...
- (void) setImage:(UIImage *)image withTitle:(NSString *)title withFont:(UIFont *)font withTitleColor:(UIColor *)color viewSpace:(CGFloat )space forState:(UIControlState)stateType
{
    //UIEdgeInsetsMake(CGFloat top, CGFloat left, CGFloat bottom, CGFloat right)
    
    CGSize titleSize = [title sizeWithFont:font];
    [self.imageView setContentMode:UIViewContentModeCenter];
    [self setImageEdgeInsets:UIEdgeInsetsMake(- 8.0 - space,
                                              0.0,
                                              0.0,
                                              -titleSize.width)];
